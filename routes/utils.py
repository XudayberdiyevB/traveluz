...
def get_routes(request, form) -> dict:
    ...
    qs = Train.objects.filter(start_train_day__gte=form.cleaned_data['travelling_time']).select_related('from_city',
                                                                                                        'to_city')
    graph = get_graph(qs)
    ...
    if not len(all_ways):
        raise ValueError('This route trains not found!')
    print(cities)
    total_time = 0
    if cities:
        ...
        if not right_ways:
            raise ValueError('This route through trains not found!')
    else:
        right_ways = all_ways
    routes = []
    all_trains = {}
    ...
    for route in right_ways:
        ...
        if total_time <= 1000000:
            routes.append(tmp)
    if not routes:
        raise ValueError('This time trains not found!')
    sorted_routes = []
    if len(routes) == 1:
